...
predictor = dlib.shape_predictor("predict_2.dat")


def detect_facial_landmarks(img, FACE_DOWNSAMPLE_RATIO=1):
    small_img = cv2.cv2.resize(
        img,
        None,
...
def warp_triangle(img1, img2, t1, t2):
    bb1 = cv2.cv2.boundingRect(np.float32([t1]))

    img1_cropped = img1[bb1[1] : bb1[1] + bb1[3], bb1[0] : bb1[0] + bb1[2]]

    bb2 = cv2.cv2.boundingRect(np.float32([t2]))
    ...
    cv2.cv2.fillConvexPoly(mask, np.int32(t2_offset), (1.0, 1.0, 1.0), cv2.cv2.LINE_AA)

    size = (bb2[2], bb2[3])

    mat = cv2.cv2.getAffineTransform(np.float32(t1_offset), np.float32(t2_offset))

    img2_cropped = cv2.cv2.warpAffine(
        img1_cropped,
        mat,
        (size[0], size[1]),
        None,
        flags=cv2.cv2.INTER_LINEAR,
        borderMode=cv2.cv2.BORDER_REFLECT_101,
    )
...
img1 = cv2.cv2.imread(sys.argv[1])
...
original_hull_index = cv2.cv2.convexHull(np.array(points1), returnPoints=False)
...
for i in count(1):
    ...
    try:
        ...
        img2_gray = cv2.cv2.cvtColor(img2, cv2.cv2.COLOR_BGR2GRAY)
        ...
        hull2_next, *_ = cv2.cv2.calcOpticalFlowPyrLK(
            img2_gray_prev,
            img2_gray,
            hull2_prev,
            ...
            winSize=(101, 101),
            maxLevel=5,
            criteria=(cv2.cv2.TERM_CRITERIA_EPS | cv2.cv2.TERM_CRITERIA_COUNT, 20, 0.001),
        )
        ...
        cv2.cv2.fillConvexPoly(mask, np.int32(original_hull2), (255, 255, 255))
        r = cv2.cv2.boundingRect(np.float32([original_hull2]))

        center = (r[0] + int(r[2] / 2), r[1] + int(r[3] / 2))
        img2 = cv2.cv2.seamlessClone(
            np.uint8(img1_warped), img2, mask, center, cv2.cv2.NORMAL_CLONE
        )

    cv2.cv2.imshow("camera", img2)
